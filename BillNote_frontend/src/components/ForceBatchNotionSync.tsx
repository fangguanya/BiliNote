...
interface ForceBatchNotionSyncProps {
  disabled?: boolean
}

const ForceBatchNotionSync: React.FC<ForceBatchNotionSyncProps> = ({ disabled = false }) => {
  ...
  // 获取所有成功的任务（包括已同步和未同步的）
  const allSuccessTasks = tasks.filter(task => task.status === 'SUCCESS')
  ...
  const handleForceBatchSync = async () => {
    ...
    try {
      // 获取所有成功任务的ID
      const taskIds = allSuccessTasks.map(task => task.id)
      ...
      if (result) {
        ...
        if (result.failed_count > 0) {
          toast.error(`${result.failed_count} 个笔记同步失败，请查看详细结果`)
        }
      }
    } catch (error) {
      console.error('强制批量同步失败:', error)
      toast.error('强制批量同步失败，请重试')
    } finally {
      setIsSyncing(false)
    }
  }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            强制同步所有笔记到Notion，包括已同步的笔记（会覆盖已有页面）
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-orange-700">
                ...
                <p>此操作将同步所有成功的笔记，包括已同步过的。已同步的笔记将创建新的Notion页面。</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-muted/50 rounded-lg">
            <div className="flex items-center justify-between text-sm">
              <span>可同步笔记总数：</span>
              <Badge variant="outline">{allSuccessTasks.length} 个</Badge>
            </div>
            <div className="flex items-center justify-between text-sm mt-1">
              <span className="text-muted-foreground">
                （包括 {tasks.filter(t => t.status === 'SUCCESS' && t.notion?.saved).length} 个已同步）
              </span>
            </div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={!isConnected || isSyncing || allSuccessTasks.length === 0 || (saveMode === 'database' && !selectedDatabaseId)}
          ...
          >
            ...
            {isSyncing ? '强制同步中...' : `强制同步 (${allSuccessTasks.length} 个)`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default ForceBatchNotionSync 